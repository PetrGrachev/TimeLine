...
export function refreshToken(refresh_token) {
  ...
  return axiosInstance.put('/auth/token', data)
    .then(response => {
...
export function registerOrg(address, city, email, lat, long, name, password, type, telephone) {
  ...
  return axiosInstance.post('/auth/registration/orgs', data)
    .then(response => {
      if (isOK(response.status)) {
        console.log("Успешный ответ:", response.data);
...
export function registerUser(email, name, last_name, city, password) {
  ...
  console.log("Запрос", data);
  return axiosInstance.post('/auth/registration/users', data)
    .then(response => {
...
export function verifyCode(code, email, id, is_org) {
  ...
  return axiosInstance.post('/auth/codes', data)
    .then(response => {
      if (response.status === 200) {
        console.log("Успешный ответ:", response.data);
        const accessToken = response.data.access_token;
